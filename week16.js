...
function makeOne() {
	console.log(document.forms.length);//Ваш код
}
...
function makeTwo() {
	console.log(document.forms[0].getAttribute('name'));//Ваш код
}
...
function makeThree() {
	console.log(document.forms[document.forms.length - 1].getAttribute('name'));//Ваш код
}
...
function makeFour() {
	...
	const string = formNames.join();
	document.getElementById('practicum4').innerHTML = string;//Ваш код
}
...
function makeFive() {
	...
	console.log(thirdForm.length);//Ваш код
}
...
function makeSix() {
	...
	console.log(secondForm.length);//Ваш код
}
...
function makeSeven() {
	...
	for (let i = 0; i < formElements.length; i += 1) {
		listElement = `- ${formElements[i].name}`;
		elementsList += listElement;
	}
	document.getElementById('practicum7').textContent = elementsList;//Ваш код
}
...
function makeEight() {
	...
	document.getElementById('practicum8').textContent = elementsList;//Ваш код
}
...
function makeNine() {
	...
	document.getElementById('practicum9').textContent = elementsList;//Ваш код
}
...
function makeTen() {
	console.log(document.forms.lastForm.elements.fourthName.value);//Ваш код
}
...
function makeEleven() {
	...
	let options = form.elements[2].children;
	
	for (let i = 0; i < options.length; i += 1) {
		optionName = `${options[i].value} `;
		optionsValues += optionName;
	}
...
function makeTwelve() {
	...
	const checkbox1 = form.elements.checkboxOne;
	const checkbox2 = form.elements.checkboxTwo;
	const checkbox3 = form.elements.checkboxThree;
...
//- Если кнопка не выбрана, выведите сообщение "Кнопка выбрана" на страницу

const paragraphThirteen = document.getElementById('practicum13');

function checkButton(e) {
	const form = document.forms.lastForm;
	const radio = form.elements[0];
	let result = '';
	
	if (radio.checked) {
		result = 'Кнопка выбрана';
	}
	else {
		result = 'Кнопка не выбрана';
	}
	console.log(result);//Ваш код
}
...
function checkOption() {
	const form = document.forms.formOne;
	const select = form.elements[2];
	let selectNumber = select.selectedIndex;
	selectNumber = 1;
...
formOne.addEventListener('submit', function (event) {
	...
	let nameInput = formOne.elements.firstName;
	let emailInput = formOne.elements.firstEmail;
	let text = [];
	
	if (nameInput.value.length < 1) {
		text.push('Поле Имя не заполнено');
	}
	if (emailInput.value.length < 1) {
		text.push(' Поле E-mail не заполнено');
	}
...
emailInput.oninput = function () {
	...
	if (emailInput.value.match(mailFormat)) {
        ...
        document.getElementById('errorMessage').textContent = "Ваш адрес электронной почты введён неверно!";
        return false;
    }//Ваш код
};
...
document.querySelector('.b-21').onclick = function (event) {
	...
	if (!isChecked) {
		...
		document.getElementById('result21').textContent = 'Проверка не пройдена';//Ваш код
	} else {
...
document.querySelector('.b-22').onclick = function (event) {
	...
	if (nameInput.value.trim() === '') {
		event.preventDefault();
		document.getElementById('result22').textContent = 'Имя не заполнено';//Ваш код
	} else {
...
//Подсказка: используйте свойство disabled
	...
	const radio23 = document.forms.lastForm.elements.fourthName;

	radio23.onchange = function() {
...
document.querySelector('.b-25').onclick = function () {
	...
	inputs.forEach(function (input) {
		input.addEventListener('focus', function () {
			input.style.borderColor = "#00ff00";//Ваш код
		});

		input.addEventListener('blur', function () {
			input.style.borderColor = '';//Ваш код
		});
	});
};
...
document.querySelector('.b-26').onclick = function (event) {
	event.preventDefault();
	document.getElementById('result26').innerHTML = document.forms.formThree.elements.thirdName.getAttribute('placeholder');//Ваш код
};
...
selectFormThree.addEventListener('change', function () {
		document.getElementById('result28').innerHTML = 'Опция выбрана';//Ваш код
});
...
function handleSubmitTwentyNine(event) {
	...
	if (nameInputThree.value && emailInputThree.value) {
		...
		console.log(emailInputThree.value);//Ваш код
	} else {
